...
/* Ensure any logo spacers generated by the model are purely invisible space */
.logo-spacer,
[class*="spacer"],
[class*="placeholder"] {{
...
"""
    ...
    if "<meta charset" not in html.lower():
        if head_open in html.lower():
            idx = html.lower().index(head_open) + len(head_open)
            html = html[:idx] + "\n" + meta_tag + html[idx:]
        ...
        with sync_playwright() as p:
            ...
            try:
                pdf_bytes = page_obj.pdf(
                    width=f"{page.width_mm:.3f}mm",
                    height=f"{page.height_mm:.3f}mm",
                    margin={"top": "0mm", "right": "0mm", "bottom": "0mm", "left": "0mm"},
                    print_background=True,
                    prefer_css_page_size=True,
                )
            except Exception:
                # If Chromium's PDF printing driver encounters a protocol error,
                # fall back to capturing a full-page PNG screenshot and building the PDF.
                png_bytes = page_obj.screenshot(full_page=True, type="png")
                browser.close()
                pil_image = Image.open(io.BytesIO(png_bytes)).convert("RGB")
                pdf_doc = pymupdf.open()
                pdf_page = pdf_doc.new_page(width=page.width_pt, height=page.height_pt)
